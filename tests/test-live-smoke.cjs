...
const EXPECTED_BUILD = '2026.07.12.1';
...
module.exports = async function run(){
  ...
  check('live page has no missing/broken resources on load', badResponses.length === 0);
  if(pageErrors.length) console.log('    errors seen:', pageErrors);
  if(badResponses.length) console.log('    bad responses seen:', badResponses);
  ...
  for(const proto of protocols){
    ...
    check(proto.value + ': live badge text is correct', result.badgeText.includes(proto.label));
    if(proto.bodyClass){
  ...
  const assetChecks = await page.evaluate(async () => {
    const urls = ['/assets/blitz-bolt.png'];
    const results = {};
